...
/**
 * McCauley 2013 Sleep Model implementation - sleephistory function provides the
 * final alertness value as per the sleep schedule and hour
 * 
 * @author ehsanebk
 */
public class BioMathModel {

	private static final double precision = .05 / 3;

	private static final double alphaw = -0.028;
	private static final double alphas = -0.26;
	private static final double betaw = -0.26;
	private static final double betas = betaw;
	private static final double muw = 0.33;
	private static final double mus = -1.5;
	private static final double etaw = 0.0074;
	private static final double tau = 24;
	private static final double fi1 = 21.2;
	private static final double Wc = 20.2;
	private static final double Ac = Wc / tau;
	private static final double etas = (Ac / (Ac - 1)) * etaw; // equation 4

	private static final double lambdas = -0.49;
	private static final double lambdaw = 0.49;
	private static final double ksi = 1.09;
	// private static final double W = 16;
	// private static final double kappat0 = (((Math.exp(lambdaw * W) *
	// Math.exp(lambdas * (tau - W))) - 1)
	// / (Math.exp(lambdaw * W) - 1));
	...
	/*
	 * returns the list of p, u and k values during time awake between hours t0
	 * and t0+t, where t is the total time awake, and all other values are
	 * initial
	 */
	...
	/*
	 * returns the list of p, u and k values during time asleep between hours t0
	 * and t0+t, where t is the total time awake, and all other values are
	 * initial
	 */
	...
	/*
	 * takes in a list of times awake and times asleep and returns the list of
	 * values for p, u, and k
	 */
	static ArrayList<ArrayList<Double>> sleepsched(ArrayList<Double> wake, ArrayList<Double> asleep, double p0,
			double u0, double k0) {
		...
		while (i < (wake.size() - 1)) {
			values = rBind(values,
					awakevalues(asleep.get(i) - wake.get(i) - precision, wake.get(i),
							values.get(values.size() - 1).get(0), // getting the
																	// p for the
																	// last row
																	// of values
							values.get(values.size() - 1).get(1), // getting the
																	// u for the
																	// last row
																	// of values
							values.get(values.size() - 1).get(2))); // getting
																	// the k for
																	// the last
																	// row of
																	// values
			values = rBind(values,
					asleepvalues(wake.get(i + 1) - asleep.get(i) - precision, asleep.get(i),
							values.get(values.size() - 1).get(0), // getting the
																	// p for the
																	// last row
																	// of values
		...
		values = rBind(values,
				awakevalues(asleep.get(i) - wake.get(i), wake.get(i), values.get(values.size() - 1).get(0), // getting
																											// the
																											// p
																											// for
																											// the
																											...
																											// of
																											// values
						values.get(values.size() - 1).get(1), // getting the u
																// for the last
																// row of values
						values.get(values.size() - 1).get(2))); // getting the k
																// for the last
																// row of values
		return values;
	}
