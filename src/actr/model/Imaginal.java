...
public class Imaginal extends Module {
	...
	void update() {
		...
		if (chunk != null && chunk.isRequest()) {
			...
			model.getBuffers().setSlot(Symbol.imaginalState, Symbol.state,
					Symbol.busy);
			model.getBuffers().setSlot(Symbol.imaginalState, Symbol.buffer,
					Symbol.requested);
			model.addEvent(new Event(model.getTime() + imaginalDelay,
					"imaginal", "set-buffer [" + chunk.getName() + "]") {
				@Override
				public void action() {
					model.getBuffers().set(Symbol.imaginal, chunk);
					model.getBuffers().setSlot(Symbol.imaginalState,
